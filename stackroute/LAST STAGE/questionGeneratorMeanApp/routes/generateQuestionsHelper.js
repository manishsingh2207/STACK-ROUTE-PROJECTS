...
module.exports=function makeClaimAndPrepareArrayOfUris (req, res, next){
  ...
  request(searchUri, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      // console.log(searchUri);
      for (var i = 0; i < numberOfQuestions; i++) { //set 200 as limit here...
...
var concurrency;
if(arrayOfUri.length<1000){
  concurrency=arrayOfUri;
}
else{
  concurrency=1000;
}
Promise.map(arrayOfUri, function(uri){
    // rp(uri)
    //     .then(function (body) {
    //       arrayOfObjectsForEachEntity=[];
    ...
    //         // Crawling failed...
    //     })
    //     //Promise.all(arrayOfRequestsToBePromised).then(getEntityValuePairList());
    return   rp(uri)
          .then(function (body) {
            arrayOfObjectsForEachEntity=[];
              var obj = JSON.parse(body);
              arrayOfObjectsForEachEntity.push(obj["entities"]);
              getEntityValuePairList();
              //console.log(obj["entities"]);
          })
          .catch(function (err) {
              // Crawling failed...
  ...
  if(dataType==="wikibase-item"){
    ...
    console.log("inside getNameForOptionPid");

  }
  else {
    generateQuestionsJsonToBeSentBackToClient(objectToContainVariableAndAnswer,imageArray,questionStub,res,topicIds);
    console.log("inside generateQuestionsJsonToBeSentBackToClient");
  }


});

function getEntityValuePairList() {
...
function getNameForOptionPid(objectToContainVariableAndAnswer,imageArray,questionStub,res,topicIds){
  ...
  var arrayOfRequestsToBePromised=[];
  var eachEntityName={};
  console.log("inside getNameForOptionPid 2nd time");
  console.log(objectToContainVariableAndAnswer);
  var tempObjectToContainVariableAndAnswer=objectToContainVariableAndAnswer;
  for (var firstTempVarForKeyRunover in tempObjectToContainVariableAndAnswer) {
    if (objectToContainVariableAndAnswer.hasOwnProperty(firstTempVarForKeyRunover)) {
      console.log(firstTempVarForKeyRunover);
      for (var i = 0; i < tempObjectToContainVariableAndAnswer[firstTempVarForKeyRunover].length; i++) {
        searchUri='https://www.wikidata.org/wiki/Special:EntityData/Q'+tempObjectToContainVariableAndAnswer[firstTempVarForKeyRunover][i]+'.json';
        searchUriList.push(searchUri);
        }
      }
    }


        console.log(searchUriList);

        Promise.map(searchUriList, function(uri){
          ...
          tempObjectToContainVariableAndAnswer=objectToContainVariableAndAnswer;
          ...
          console.log(objectToContainVariableAndAnswer);
        generateQuestionsJsonToBeSentBackToClient(objectToContainVariableAndAnswer,imageArray,questionStub,res,topicIds)
          console.log("done"+Object.keys(objectToContainVariableAndAnswer).length);


          //console.log(objectToContainVariableAndAnswer);
        });

        function getEntityAnswerList(){
          console.log("<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>");
          console.log(arrayOfObjectsForEachEntity);
          //console.log("arrayOfObjectsForEachEntity.length"+arrayOfObjectsForEachEntity.length);
          for (var i = 0; i < arrayOfObjectsForEachEntity.length; i++) {
            // console.log("i = "+i);
