                        ...
                        if (dataType === "wikibase-item") {
                            getNameForOptionPid(objectToContainVariableAndAnswer)
                                .then(function(resultsArray) {
                                    ...
                                    generateQuestionsJsonToBeSentBackToClient(objectToContainVariableAndAnswer, imageArray,questionStub)
                                        .then(function(resultsArray) {
                                          console.log(resultsArray);
                                          ...
                                          console.log("end... " + endIndex);
                                            if (endIndex < numberOfQuestions) {
                                              ...
                                              generateDistractors('./tempFileToStoreIntermediary.json',topicIds,poolOfAllUniqueAnswers,res);
                                            }
                                        });
...
function generateQuestionsJsonToBeSentBackToClient(objectToContainVariableAndAnswer, imageArray, questionStub) {
    var deferred = Promise.defer();
...
function generateArrayofImageUrl(arrayOfImageUrlFromWiki) { // function to generate final Image Url from Image Text of Wikidata
 ...
 function generateMd5Hash(stringToBeHashed) {  // function to generateMd5Hash
   ...
   function generateDistractors(fileLocation,topicIds,poolOfAllUniqueAnswers,res){
