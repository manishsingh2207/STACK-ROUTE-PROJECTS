...
function($scope,$http){
  ...
  for (var i = 0; i < response.data.length; i++) {
    ...
    $scope.topicsList[response.data[i]["_id"]]=response.data[i]["topicName"];
  }
  }, function errorCallback(response) {
  ...
  $http({method: 'Post', url: '/generateQuestions', timeout : 100000000,  data: {data:dataToBeSentToServer }}).
    success(function(data, status, headers, config) {
      ...
      for (var i = 0; i < $scope.generateQuestionsList.length; i++) {
        ...
        $scope.generateQuestionsList[i]["options"]=$scope.generateQuestionsList[i]["options"];
